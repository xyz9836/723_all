...
def dashboard():
    ...
    if 'boxes_sel' not in user_info.keys():
        user_info['boxes_sel'] = ["实时上传速度", "实时下载速度", "今日矿场产量", "本周水晶收益", "本月水晶收益", "今日宝箱转盘"]
...
def DoD_income_xunlei():
    ...
    if miner_summary(username)['total_count'] != 0:
        ave_income_value = now_income_value / miner_summary(username)['total_count']
    else:
        ave_income_value = 0
    upload_data_value = str(int(3600*s_sum/1024/10.24)/100) + 'GB'
    ...
    if 'is_show_speed_data' in user_info.keys() and user_info['is_show_speed_data'] == False:
        return Response(json.dumps(dict(series=[yesterday_series, today_series, yesterday_speed_series, today_speed_series],data=dict(last_day_income=convert_to_yuan(yesterday_last_value), dod_income_value=convert_to_yuan(dod_income_value),expected_income=convert_to_yuan(expected_income), ave_income_value=convert_to_yuan(ave_income_value), upload_data_value=upload_data_value))), mimetype='application/json')
    else:
        return Response(json.dumps(dict(series=[yesterday_series, today_series],data=dict(last_day_income=convert_to_yuan(yesterday_last_value), dod_income_value=convert_to_yuan(dod_income_value),expected_income=convert_to_yuan(expected_income), ave_income_value=convert_to_yuan(ave_income_value),  upload_data_value=upload_data_value))), mimetype='application/json')
