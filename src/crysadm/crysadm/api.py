...
def api_silver_account(cookies):
    cookies['origin'] = '4' if len(cookies.get('sessionid')) == 128 else '1'
    r = api_get("/index.php?r=kcv3/silveraccount&from=1&version=3.1.1", cookies=cookies)
    return r

# 获取星域存储相关信息
def ubus_cd(session_id, account_id, action, out_params, url_param=None, spec_sort=False):
    ...
    try:
        if spec_sort:
            data = {"0_jsonrpc": "2.0", "1_id": 1, "2_method": "3_call", "4_params": params}
            body = json.dumps(data, sort_keys=True, separators=(',',':'))
            for i in range(0,9):
                body = body.replace('%d_' % i,'')
            return body
            body = dict(data=body, action='onResponse%d' % int(time.time() * 1000))
        else:
